...
export default function ResourcesTab({ selectedVertical }) {
  ...
  return (
    <div>
      ...
      {items.length === 0 ? (
        <div style={{ fontSize: 14, color: "var(--text-faint)", padding: "20px 0" }}>No resources available for this vertical yet.</div>
      ) : (
        <div style={{
          display: "grid",
          ...
          {items.map((item, i) => (
            <a
              ...
              onMouseEnter={e => { e.currentTarget.style.borderColor = "var(--hover-border)"; e.currentTarget.style.boxShadow = "0 1px 4px rgba(0,0,0,0.08)"; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = "var(--border-mid)"; e.currentTarget.style.boxShadow = "none"; }}
            >
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--text-faint)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, marginTop: 2 }}>
              ...
              <div style={{ minWidth: 0 }}>
                ...
                {item.metric && (
                  <div style={{ fontSize: 12, color: "var(--text-faint)", lineHeight: 1.4, marginTop: 4 }}>{item.metric}</div>
                )}
              </div>
            </a>
          ))}
